...
class OtpPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `otp`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM otp WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO otp (date, title, link, content)"
